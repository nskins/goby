...
RSpec.describe do
  ...
  before(:each) do
    ...
    @player = Player.new(stats: { max_hp: 10, hp: 3 },
                         inventory: [ Couple[Food.new(name: "Banana", recovers: 5), 1],
                                      Couple[Food.new(name: "Onion", disposable: false), 1],
                                      Couple[Item.new(name: "Big Book of Stuff"), 1],
                                      Couple[Helmet.new, 1] ],
                         map: @map,
                         location: Couple[0, 0])
  end
  ...
  context "interpret command" do
    ...
    context "lowercase" do
      it "should correctly move the player around" do
        ...
        expect(@player.location).to eq Couple[1, 0]
        interpret_command("d", @player)
        expect(@player.location).to eq Couple[1, 1]
        interpret_command("w", @player)
        expect(@player.location).to eq Couple[0, 1]
        interpret_command("a", @player)
        expect(@player.location).to eq Couple[0, 0]
      end
